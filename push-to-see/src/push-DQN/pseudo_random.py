...
def main():
    ...
    logger.save_pseudo_random_notice()
    ...
    def process_actions():
        while True:
            if nonlocal_variables['executing_action']:
                ...
                #
                # else:
                print('Strategy:' + BColors.WARNING + 'PSEUDO RANDOM MODEL' + BColors.ENDC)
                ...
                # trainer.predicted_value_log.append()
                # logger.write_to_log('predicted-value', trainer.predicted_value_log)

                # Compute 3D position of pixel
                print('Action: %s at (rotation no = %d and (x,y) = (%d, %d))' % (nonlocal_variables['primitive_action'], nonlocal_variables['best_pix_ind'][0],
                                                      nonlocal_variables['best_pix_ind'][2], nonlocal_variables['best_pix_ind'][1]))
                ...
                # Save executed primitive --> 0 for exploit, 1 for explore -- 2 for pseudo-random
                trainer.executed_action_log.append([2, nonlocal_variables['best_pix_ind'][0],
    ...
    while True:
        print(BColors.WARNING + '\n%s --> iteration: %d' % ('Testing Pseudo Random Model!', trainer.iteration) +
        ...
        highest_point = np.unravel_index(np.argmax(valid_depth_heightmap), valid_depth_heightmap.shape)
        rand_direction = np.random.random_integers(0, trainer.model.num_rotations-1)
        rotation_angle = np.deg2rad(rand_direction * (360.0/trainer.model.num_rotations))
        # hightmap --> [y axis, x axis]!!
        direction = np.asarray([np.sin(rotation_angle), np.cos(rotation_angle)])
        # the start position is 10 points away from the highest position in the opposite pushing direction.
        # if it's not in the limits ([0, 223]) then take max or min accordingly
        push_start_position = np.maximum(np.minimum(highest_point - (direction * 10), 223), 0)
        nonlocal_variables['best_pix_ind'] = (rand_direction, int(push_start_position[0]), int(push_start_position[1]))
        ...
        if nonlocal_variables['session_first_loop']:
            ...
            # depth_converted = np.round((depth_img_raw * 10000) / 255).astype(np.uint8)
            # depth_converted = np.repeat(depth_converted.reshape(480, 640, 1), 3, axis=2)
        ...
        if 'prev_color_img' in locals():
            ...
            if not success_without_push:
                ...
                #                                                       nonlocal_variables['session_success'], session_counter)
                #
                ...
                #
                diff = seg_reward - prev_seg_reward
                current_reward = (diff * 10) ** 2
                if diff < 0:
                    current_reward *= -1
                seg_vals = [seg_reward, prev_seg_reward, diff, current_reward]
                ...
                #     explore_prob = max(explore_prob * np.power(0.9998, trainer.iteration), min_rate_decay)
